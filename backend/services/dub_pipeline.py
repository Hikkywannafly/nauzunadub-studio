...
async def ingest_pipeline(
    ...
    try:
        ...
        if not ffmpeg:
            msg = (
                "ffmpeg not found. Cài `imageio-ffmpeg` (uv sync), hoặc set "
                "FFMPEG_PATH env var đến binary ffmpeg."
            )
            logger.error("extract failed for job %s: %s", job_id, msg)
            yield prep_event("error", stage="extract", error=msg)
            return
        if not os.path.isfile(video_path):
            msg = f"Source video missing on disk: {video_path}"
            logger.error("extract failed for job %s: %s", job_id, msg)
            yield prep_event("error", stage="extract", error=msg)
            return
        try:
            p, stdout, stderr = await run_proc([
            ...
            if p.returncode != 0:
                ...
                if not err_txt:
                    err_txt = stdout.decode(errors="replace").strip()
                if not err_txt:
                    err_txt = f"ffmpeg exited with code {p.returncode} (no stderr)"
                logger.error(
                    "ffmpeg extract failed (job=%s, rc=%s, ffmpeg=%s, video=%s):\n%s",
                    job_id, p.returncode, ffmpeg, video_path, err_txt,
                )
                raise Exception(err_txt[:500])
        except asyncio.CancelledError:
            raise
        except Exception as e:
            err_msg = str(e) or f"{type(e).__name__}: <empty>"
            logger.exception("extract stage crashed for job %s (ffmpeg=%s, video=%s)",
                             job_id, ffmpeg, video_path)
            yield prep_event("error", stage="extract", error=err_msg[:300])
            return
